...
class CoapServer:
    coap_discovery_ipv6 = ['FF02::158']
    coap_discovery_ipv4 = ['224.0.1.187']
    coap_timeout = 25
    ...
    async def send_request(self, message, remote, **kwargs):
        self.log.debug('{0}:{1}{2}'.format(
            remote[0], remote[1], message.opt.uri_path))
        self.answer[message.token] = dict(
            request=message,
    ...
    def send_message(self, message, remote):
        try:
            ...
            # remote = (message.opt.uri_host, message.opt.uri_port)
            if socket.getaddrinfo(remote[0], remote[1])[0][0] == socket.AF_INET6:
                transport = self.endpoint['IPv6']['transport']
            else:
